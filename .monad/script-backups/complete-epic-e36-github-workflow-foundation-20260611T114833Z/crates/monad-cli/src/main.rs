...
enum CliCommand {
    ...
    /// Render or write policy and approval-gate evidence.
    Policy {
        /// Whether to run in dry-run mode.
        dry_run: bool,
    ...
    Upgrade {
        ...
        /// Whether to apply guarded generated upgrade writes.
        yes: bool,
    ...
    /// Synchronize declared repository intent with discovered state.
    Sync {
        /// Whether to run in dry-run mode.
        dry_run: bool,
    ...
    WebWorkbenchPlan {
        ...
        /// Whether to write generated web workbench evidence.
        yes: bool,
...
impl CliCommand {
    ...
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, String> {
        ...
        let mut requested_release_version: Option<String> = None;
        ...
        while let Some(argument) = arguments.next() {
            ...
            if let Some(value) = argument.strip_prefix("--release-version=") {
                if requested_release_version.is_some() {
                    return Err("--release-version may only be provided once".to_string());
                }
                requested_release_version = Some(value.to_string());
                continue;
            }
        ...
        if yes
            ...
            && parts.first().copied() != Some("web-workbench-plan")
            && parts.first().copied() != Some("web-workbench")
            && parts.first().copied() != Some("web-ui")
        ...
        if requested_release_version.is_some() && parts.first().copied() != Some("release") {
            return Err("--release-version is only supported for release command".to_string());
        }
        ...
        match parts.as_slice() {
            ...
            ["upgrade"] => {
                reject_write_for_non_context(write)?;
                require_upgrade_mode(dry_run, yes)?;
                let output_format = parse_output_format_or_default(requested_format.as_deref())?;
                Ok(Self::Upgrade {
                    dry_run,
                    yes,
            ...
            ["upgrade", other, ..] => {
                reject_write_for_non_context(write)?;
                Err(format!("unknown upgrade argument: {other}"))
            }
            ["release"] => {
                reject_write_for_non_context(write)?;
                require_release_mode(dry_run, yes)?;
                let output_format = parse_output_format_or_default(requested_format.as_deref())?;
                Ok(Self::Release {
                    dry_run,
            ...
            ["web-workbench-plan"] | ["web-workbench"] | ["web-ui"] => {
                reject_write_for_non_context(write)?;
                require_web_workbench_plan_mode(dry_run, yes)?;
                let output_format = parse_output_format_or_default(requested_format.as_deref())?;
                Ok(Self::WebWorkbenchPlan {
                    dry_run,
                    yes,
                    output_format,
                })
            }
            ["web-workbench-plan", other, ..]
            | ["web-workbench", other, ..]
            | ["web-ui", other, ..] => {
                reject_write_for_non_context(write)?;
                Err(format!("unknown web-workbench-plan argument: {other}"))
            }
...
fn run(args: impl IntoIterator<Item = String>) -> Result<String, String> {
    ...
    match command {
        ...
        CliCommand::Upgrade {
            ...
            yes,
            output_format,
        } => render_upgrade(dry_run, yes, output_format),
        CliCommand::Release {
            dry_run,
            version,
            output_format,
        ...
        CliCommand::WebWorkbenchPlan {
            ...
            yes,
            output_format,
        } => render_web_workbench_plan(dry_run, yes, output_format),
...
/// Requires exactly one policy mode.
...
fn require_upgrade_mode(dry_run: bool, yes: bool) -> Result<(), String> {
    match (dry_run, yes) {
        (true, false) | (false, true) => Ok(()),
        (false, false) => {
...
fn require_web_workbench_plan_mode(dry_run: bool, yes: bool) -> Result<(), String> {
    match (dry_run, yes) {
        (true, false) | (false, true) => Ok(()),
        (false, false) => Err(
            "web-workbench-plan currently requires either --dry-run to preview or --yes to write generated web workbench evidence".to_string(),
        ),
        (true, true) => Err("web-workbench-plan accepts either --dry-run or --yes, not both".to_string()),
    }
}
...
fn require_release_mode(dry_run: bool, yes: bool) -> Result<(), String> {
    match (dry_run, yes) {
        (true, false) => Ok(()),
        (false, false) => Err(
            "release currently requires --dry-run; tag creation and publishing are not implemented"
                .to_string(),
        ),
...
fn help_text() -> String {
    [
        ...
        "  upgrade --yes                           Apply guarded generated upgrade metadata",
        "  release --dry-run --format=json         Render release readiness as JSON",
        "  doctor --format=json                     Render doctor report as JSON",
        ...
        "  web-workbench-plan --dry-run             Preview local web workbench plan",
        "  web-workbench-plan --dry-run --format=json Preview web workbench plan as JSON",
        ...
        "",
        "Safety notes:",
        "  doctor is read-only and does not install tools or mutate the environment.",
...
fn render_web_workbench_plan(
    ...
    if dry_run {
        let plan = monad_core::build_web_workbench_plan(&root);
        return match output_format {
            OutputFormat::Text => Ok(monad_core::render_web_workbench_plan(&plan)),
            OutputFormat::Json => Ok(monad_core::render_web_workbench_plan_json(&plan)),
        };
    }

    if yes {
        let result =
            monad_core::write_web_workbench_evidence(&root).map_err(|error| error.to_string())?;
        return Ok(monad_core::render_web_workbench_apply_result(&result));
    }

    Err("web-workbench-plan currently requires either --dry-run to preview or --yes to write generated web workbench evidence".to_string())
}
...
fn render_upgrade(dry_run: bool, yes: bool, output_format: OutputFormat) -> Result<String, String> {
    let root = std::env::current_dir().map_err(|error| error.to_string())?;

    if dry_run {
        let plan = build_upgrade_plan(&root);
        return match output_format {
            OutputFormat::Text => Ok(render_upgrade_plan(&plan)),
            OutputFormat::Json => Ok(render_upgrade_plan_json(&plan)),
        };
    }

    if yes {
        let result = apply_upgrade_plan(&root)?;
        return Ok(render_upgrade_apply_result(&result));
    }
...
fn render_release(
    ...
    let root = std::env::current_dir().map_err(|error| error.to_string())?;
    let options = ReleasePlanOptions::new(version);
    let plan = build_release_plan(root, &options);
...
fn render_doctor(output_format: OutputFormat) -> Result<String, String> {
    let report = run_doctor();

    match output_format {
        OutputFormat::Text => Ok(render_doctor_report(&report)),
        OutputFormat::Json => Ok(render_doctor_report_json(&report)),
    }
}
...
mod tests {
    ...
    fn init_requires_dry_run_or_yes() {
        let error = parse_arguments(&["monad", "init"]).expect_err("init without mode should fail");
    ...
    #[test]
    fn add_language_yes_command_parses() {
        assert_eq!(
            parse_arguments(&[
                "monad",
                "add",
            ...
            .expect("add language --yes should parse"),
            CliCommand::Add {
                kind: ComponentKind::Service,
                name: ComponentName::parse("api").expect("test name should parse"),
                language: Some(ComponentLanguage::Rust),
    ...
    fn web_workbench_plan_dry_run_command_parses() {
        assert_eq!(
            parse_arguments(&["monad", "web-workbench-plan", "--dry-run"]),
            Ok(CliCommand::WebWorkbenchPlan {
                dry_run: true,
                yes: false,
                output_format: OutputFormat::Text,
            })
        ...
        assert_eq!(
            parse_arguments(&["monad", "web-workbench-plan", "--dry-run", "--format=json"]),
            Ok(CliCommand::WebWorkbenchPlan {
                dry_run: true,
                yes: false,
                output_format: OutputFormat::Json,
    ...
    #[test]
    fn web_workbench_plan_aliases_parse() {
        assert_eq!(
            parse_arguments(&["monad", "web-workbench", "--dry-run"]),
            Ok(CliCommand::WebWorkbenchPlan {
                dry_run: true,
                yes: false,
                output_format: OutputFormat::Text,
        ...
        assert_eq!(
            parse_arguments(&["monad", "web-ui", "--dry-run"]),
            Ok(CliCommand::WebWorkbenchPlan {
                dry_run: true,
                yes: false,
                output_format: OutputFormat::Text,
    ...
    #[test]
    fn web_workbench_plan_yes_command_parses() {
        assert_eq!(
            parse_arguments(&["monad", "web-workbench-plan", "--yes"]),
            Ok(CliCommand::WebWorkbenchPlan {
                dry_run: false,
                yes: true,
                output_format: OutputFormat::Text,
    ...
    fn ai_context_requires_mode() {
        let error =
            parse_arguments(&["monad", "ai-context"]).expect_err("ai-context should require mode");
    ...
    fn upgrade_dry_run_command_parses() {
        assert_eq!(
            parse_arguments(&["monad", "upgrade", "--dry-run"])
        ...
        assert_eq!(
            parse_arguments(&["monad", "upgrade", "--dry-run", "--format=json"])
                .expect("upgrade dry-run json should parse"),
            CliCommand::Upgrade {
                dry_run: true,
                yes: false,
                output_format: OutputFormat::Json,
    ...
    #[test]
    fn upgrade_yes_command_parses() {
        assert_eq!(
            parse_arguments(&["monad", "upgrade", "--yes"]).expect("upgrade yes should parse"),
            CliCommand::Upgrade {
                dry_run: false,
                yes: true,
                output_format: OutputFormat::Text,
    ...
    #[test]
    fn upgrade_requires_mode() {
        let error =
            parse_arguments(&["monad", "upgrade"]).expect_err("upgrade should require mode");

        assert!(error.contains("upgrade currently requires either --dry-run"));
    }

    #[test]
    fn release_command_parses_text_and_json_formats() {
        assert_eq!(
            parse_arguments(&["monad", "release", "--dry-run"]).expect("release should parse"),
            CliCommand::Release {
                dry_run: true,
                version: None,
                output_format: OutputFormat::Text,
        ...
        assert_eq!(
            parse_arguments(&["monad", "release", "--dry-run", "--format=json"])
        ...
        assert_eq!(
            parse_arguments(&["monad", "release", "--dry-run", "--release-version=0.1.0"])
                .expect("release version override should parse"),
            CliCommand::Release {
                dry_run: true,
    ...
    #[test]
    fn release_requires_dry_run() {
        let error =
            parse_arguments(&["monad", "release"]).expect_err("release should require dry-run");

        assert!(error.contains("release currently requires --dry-run"));
    }

    #[test]
    fn doctor_command_parses_text_and_json_formats() {
        assert_eq!(
            parse_arguments(&["monad", "doctor"]).expect("doctor should parse"),
            CliCommand::Doctor {
                output_format: OutputFormat::Text,
            }
        );
        ...
        assert_eq!(
            parse_arguments(&["monad", "doctor", "--format=json"])
