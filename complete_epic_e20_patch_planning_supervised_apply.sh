...
# complete_epic_e20_patch_planning_supervised_apply.sh
#
# Implements Epic 20 — Patch Planning and Supervised Apply Foundation.
# This script is intentionally local-first, deterministic, and supervised.
# It backs up touched files before editing and refuses to run outside a Monad repo.

SCRIPT_NAME="complete_epic_e20_patch_planning_supervised_apply"
...
backup_path crates/monad-core/src/patch.rs
backup_path docs/patch/README.md
backup_path docs/roadmap/epic-20-patch-planning-supervised-apply.md
backup_path tools/scripts/verify-patch.sh
backup_path tools/scripts/verify-e20.sh

mkdir -p docs/patch docs/roadmap tools/scripts crates/monad-core/src

cat > crates/monad-core/src/patch.rs <<'RS'
...
use crate::policy::{
    ...
    PolicyFindingSeverity, check_file_operation, gated_generated_write,
};
use std::fs;
...
#[must_use]
pub fn validate_patch_change_set(root: &Path, change_set: &PatchChangeSet) -> PatchValidationReport {
...
pub fn render_patch_apply_result(result: &PatchApplyResult) -> String {
    let mut lines = vec![
        ...
        format!("  planned_changes: {}", result.plan().change_set().changes().len()),
        format!("  validation_blocked: {}", result.plan().validation().is_blocked()),
        format!("  policy_blocked: {}", result.plan().approval_plan().is_blocked()),
        String::new(),
    ...
    lines.push("  - User-owned source mutation remains blocked in this foundation slice.".to_string());
...
if "pub mod patch;" not in lib:
    marker = "pub mod output;\n"
    if marker not in lib:
        raise SystemExit("Could not find insertion point for pub mod patch in monad-core lib.rs")
    lib = lib.replace(marker, marker + "pub mod patch;\n", 1)

patch_pub_use = '''pub use patch::{
    PatchApplyResult, PatchChangeKind, PatchChangeSet, PatchFileChange, PatchPlan,
    PatchValidationFinding, PatchValidationReport, PatchValidationSeverity,
    apply_patch_plan, build_patch_plan, render_patch_apply_result,
    render_patch_apply_result_json, render_patch_plan, render_patch_plan_json,
};
'''

if "pub use patch::{" not in lib:
    marker = "pub use output::{\n"
    if marker not in lib:
        raise SystemExit("Could not find insertion point for patch exports in monad-core lib.rs")
    lib = lib.replace(marker, patch_pub_use + marker, 1)
...
if "Patch {" not in main:
    marker = "\n    /// Generate provider-agnostic AI context and memory artifacts.\n"
    if marker not in main:
        raise SystemExit("Could not find enum insertion point for Patch command")
    main = main.replace(marker, patch_variant + marker, 1)

if '&& parts.first().copied() != Some("patch")' not in main:
    before = '''            && parts.first().copied() != Some("policy")
            && parts.first().copied() != Some("sync")'''
    after = '''            && parts.first().copied() != Some("policy")
            && parts.first().copied() != Some("patch")
            && parts.first().copied() != Some("sync")'''
    if before not in main:
        raise SystemExit("Could not find --yes allow-list insertion point")
    main = main.replace(before, after, 1)
...
'''
    if marker not in main:
        raise SystemExit("Could not find parse insertion point for patch command")
    main = main.replace(marker, patch_parse + marker, 1)
...
if "=> render_patch(dry_run, yes, output_format)," not in main:
    marker = '''        CliCommand::AiContext {
'''
    if marker not in main:
        raise SystemExit("Could not find run insertion point for patch command")
    main = main.replace(marker, patch_run + marker, 1)
...
if "fn require_patch_mode" not in main:
    marker = "/// Requires exactly one policy mode.\n"
    if marker not in main:
        raise SystemExit("Could not find require_patch_mode insertion point")
    main = main.replace(marker, require_patch + marker, 1)
...
if "fn render_patch(" not in main:
    marker = "/// Renders or writes policy evidence.\n"
    if marker not in main:
        raise SystemExit("Could not find render_patch insertion point")
    main = main.replace(marker, render_patch + marker, 1)
...
patch_tests = r'''
    ...
    fn patch_dry_run_command_parses() {
        assert_eq!(
            parse_arguments(&["monad", "patch", "--dry-run"])
                .expect("patch dry-run should parse"),
            CliCommand::Patch {
                dry_run: true,
                yes: false,
...
if "fn patch_dry_run_command_parses" not in main:
    idx = main.rfind("}\n")
    if idx == -1:
        raise SystemExit("Could not find end of main.rs test module")
    main = main[:idx] + patch_tests + main[idx:]

main_path.write_text(main)
PY

cat > docs/patch/README.md <<'MD'
...
cat > docs/roadmap/epic-20-patch-planning-supervised-apply.md <<'MD'
...
cat > tools/scripts/verify-patch.sh <<'SH_VERIFY_PATCH'
#!/usr/bin/env bash
set -euo pipefail
...
cat > tools/scripts/verify-e20.sh <<'SH_VERIFY_E20'
#!/usr/bin/env bash
set -euo pipefail
...
write_executable tools/scripts/verify-patch.sh
write_executable tools/scripts/verify-e20.sh

# Let rustfmt normalize all Rust edits made by this script.
cargo fmt

echo "E20 patch planning and supervised apply foundation files written."
