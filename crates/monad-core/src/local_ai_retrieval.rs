...
pub fn render_local_retrieval_plan(plan: &LocalAiRetrievalPlan) -> String {
    let mut lines = vec![
        ...
        format!(
            "  vector_store: {}",
            plan.contract().vector_store().as_str()
        ),
...
mod tests {
    ...
    fn local_embedding_is_deterministic() {
        let left = embed_text_locally(
            "local governance memory",
            LocalAiEmbeddingProvider::DeterministicLocal,
        );
        let right = embed_text_locally(
            "local governance memory",
            LocalAiEmbeddingProvider::DeterministicLocal,
        );
    ...
    fn documents_are_discovered_locally() {
        ...
        assert!(
            documents
                .iter()
                .any(|document| document.path() == Path::new("Cargo.toml"))
        );
        assert!(
            documents
                .iter()
                .any(|document| document.path() == Path::new("docs/retrieval.md"))
        );
